...
export const HDFC_TEMPLATE: BankTemplateConfig = {
    ...
    patterns: [
        ...
        },
        {
            ...
            regex: String.raw`Rs\.?\s*(?<amount>[\d,]+(?:\.\d{2})?)\s*(?:is\s+)?(?:successfully\s+)?credited\s+to\s+(?:your\s+)?(?:account|a\/c)\s*\**(?<account>\d{4})\s+by\s+VPA\s+(?<description>.*?)\s+on\s+(?<date>\d{2}-\d{2}-\d{2,4})`,
            dateFormat: 'dd-MM-yy'
        },
        {
            type: TransactionType.DEBIT,
            // HDFC UPI/VPA Debit: "Rs. 1.00 has been debited from account 0072 to VPA ... on 08-02-26."
            regex: String.raw`Rs\.?\s*(?<amount>[\d,]+(?:\.\d{2})?)\s*(?:is\s+|has\s+been\s+)?debited\s+from\s+(?:your\s+)?(?:account|a\/c)\s*\**(?<account>\d{4})\s+to\s+VPA\s+(?<description>.*?)\s+on\s+(?<date>\d{2}-\d{2}-\d{2,4})`,
            dateFormat: 'dd-MM-yy'
        }
    ]
};
